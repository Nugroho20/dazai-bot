...
exports.run = async (client, message, args) => {
    ...
    if (!tier.includes(args[0])) {
        return message.channel.send({ embed: {
            "description": ":red_circle:| Wrong Usage m!setping `T4/T5/T6 <@role>`",
            "color": "RED"
        }});
    }
    ...
    if (args[0] === "t4") {
        ...
        if (!roles) return message.channel.send({ embed: {
            "description": ":red_circle:| Please Provide the role that you want to make it as `card-mention`",
            "color": "RED"
        }});
        ...
        message.channel.send({ embed: {
            "description": ":white_circle:| Ping for `T4` card is seted on role <@&" + roles.id + ">",
            "color": "WHITE"
        }});
    }
    if (args[0] === "T4") {
        ...
        await client.db.set(`t4ping.${message.guild.id}`, `on`)
        message.channel.send({ embed: {
            "description": ":white_circle:| Ping for `Tier 4` card is seted on role <@&" + roles.id + ">",
            "color": "WHITE"
        }});
    }
    if (args[0] === "t5") {
        ...
        message.channel.send({ embed: {
            "description": ":white_circle:| Ping for `Tier 5` card is seted on role <@&" + roles.id + ">",
            "color": "WHITE"
        }});
    }
    if (args[0] === "T5") {
    ...
    if (args[0] === "t6") {
        ...
        message.channel.send({ embed: {
            "description": ":white_circle:| Ping for `Tier 6` card is seted on role <@&" + roles.id + ">",
            "color": "WHITE"
        }});
    }
    if (args[0] === "T6") {
